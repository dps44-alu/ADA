    ...
    for(int k = 1; k <= 8; k++) {
        ...
        if(probar) {    //si se ha descartado como factible no puede ser prometedor en primer lugar
            if (S <= mejor_actual) {     //prometedor
                ...
                if(M[nueva_i][nueva_j] == -1 || sol[nueva_i][nueva_j] <= M[nueva_i][nueva_j]) {     //comprueba si vale la pena
                    explorados++;
...
int main(int argc, char *argv[]) {
    ...
    //Crear alamacén para mcp_bt
    vector<vector<int>> M(filas, vector<int>(columnas, -1));

    //Crear matriz donde se va a guardar la solución
    vector<vector<int>> sol(pesos);

    //Vector de posiciones usadas para no entrar en bucles
    ...
    int i, j = i = 0;
    int mejor_actual = numeric_limits<int>::max();  //+inf

    //Crear vector donde se guardan las posiciones que marcan el camino de la solución
    ...
    //Copiar el mejor camino e invertirlo para poder vaciarlo desde atrás en la codificación
